...
def get_displacement_errors_and_miss_rate(
    ...
    for k, v in gt_trajectories.items():
        ...
        curr_min_ade = get_ade(
            pruned_trajectories[min_idx][:horizon], v[:horizon])
        min_ade.append(curr_min_ade)
        min_fde.append(curr_min_fde)
        n_misses.append(curr_min_fde > miss_threshold)

        if forecasted_probabilities is not None:
            prob_n_misses.append(1.0 if curr_min_fde > miss_threshold else (
                1.0 - pruned_probabilities[min_idx]))
            prob_min_ade.append(
                min(
...
class FormatData:
    ...
    def __call__(self, data, predictions):
        '''
            format the data for argoverse evaluation program
        ...
        for i, name in enumerate(names):
            ...
            _pred_traj = transform_coord(pred_trajs[i], -theta[i])
            _pred_traj = _pred_traj + togloble[i].numpy()

            # Renormalized groundtruth
            gt = data['FUTURE'][i][0,:,:2].detach().cpu().numpy()
            gt = gt.cumsum(axis=-2)
            gt = transform_coord(gt, -theta[i])
            gt = gt + togloble[i].numpy()
    ...
    def results(self):
        return dict(
            forecasted_trajectories=self.forecasted_trajectories,
            gt_trajectories=self.gt_trajectories,
            city_names=self.city_names,
            forecasted_probabilities=self.forecasted_probabilities)


class LaneGCNFormatData:
    def __init__(self):
    ...
    def __call__(self, data, predictions):
        ...
        # format predictions
        pred_trajs = [x[0].detach().cpu().numpy() for x in predictions['prediction']]
        pred_confs = [x[0].detach().cpu().numpy() for x in predictions['confidence']]
        gts = [g[0].detach().cpu().numpy() for g in data['gt_futures']]

        # Save data
        for i, name in enumerate(names):
            # Renormalized groundtruth
            self.city_names[name] = city_name[i]
            self.gt_trajectories[name] = gts[i]
            self.forecasted_trajectories[name] = pred_trajs[i]
            self.forecasted_probabilities[name] = pred_confs[i]
